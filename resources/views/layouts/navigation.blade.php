...
<div class="flex antialiased flex-col w-80 text-white hidden lg:flex">
    <div class="fixed inset-y-0 z-10 flex w-80">
        <!-- CURVA MENU -->
        <svg class="absolute inset-0 w-full h-full text-customblue" style="filter: drop-shadow(20px 0 10px #00000030)"
        ...
        <div class="z-10 flex flex-col flex-1">
            ...
            <nav class="flex flex-col flex-1 w-80 mt-2 p-8  ">
                <!-- Index -->
                <a href="{{ route('dashboard') }}" id="dashboard_nav" class="pt-2 pb-2 mr-14 rounded-lg hover:bg-blue-800 ">
                ...
                <!-- Publicaciones -->
                <a href="{{ route('posts.index') }}" id="posts_nav"
                    class="pt-2 pb-2 mr-4 p-2 mt-2 rounded-lg  hover:bg-blue-800">
                    <i class="fi-sr-images text-2xl pr-2"></i>
                    <span class="text-xl">Publicaciones</span>
                </a>
                <!-- Lugares -->
                <a href="{{ route('places.index') }}" id="places_nav"
                    class="pt-2 pb-2 mr-8 p-2 mt-2 rounded-lg hover:bg-blue-800">
                    <i class="fi-sr-map-marker text-2xl pr-2"></i>
                    <span class="text-xl">Lugares</span>
                </a>
                <!-- Cuenta -->
                <a href="#" id="account_nav" class="pt-2 pb-2 mr-14 p-2 mt-2 rounded-lg hover:bg-blue-800">
            ...
            <div class="flex-shrink-0 pl-4 text-xl">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button
                        class="flex items-center p-3 ml-5 mb-8 space-x-2 rounded-lg px-3 py-2 text-customred cursor-pointer hover:bg-red-600 hover:text-white shadow"
...
<div class="w-full lg:hidden">
    <div class="fixed z-50 w-full h-16 max-w-lg -translate-x-1/2 bg-customblue rounded-tl-lg rounded-tr-lg bottom-0 left-1/2">
        <div class="grid h-full max-w-lg grid-cols-5 mx-auto">
            <!-- Index -->
            <a href="{{ route('dashboard') }}" id="dashboard_div"
                class="inline-flex flex-col items-center justify-center px-5 rounded-tl-lg group hover:bg-blue-800">
                <i class="text-xl text-white fi-sr-home"></i>
                <span class="text-xs text-white">Inicio</span>
            </a>
            <!-- Archivos -->
            <a href="{{ route('files.index') }}" id="files_div"
                class="inline-flex flex-col items-center justify-center px-5 group hover:bg-blue-800">
                <i class="fi-sr-file-image text-xl text-white"></i>
                <span class="text-xs text-white">Archivos</span>
            </a>
            <!-- Publicaciones -->
            <a href="{{ route('posts.index') }}" id="posts_div"
                class="inline-flex flex-col items-center justify-center px-5 group hover:bg-blue-800">
                <i class="fi-sr-images  text-xl text-white"></i>
                <span class="text-xs text-white">Publicaciones</span>
            </a>
            <!-- Lugares -->
            <a href="{{ route('places.index') }}" id="places_div"
                class="inline-flex flex-col items-center justify-center px-5 group hover:bg-blue-800">
            ...
            <!-- Cuenta -->
            <button type="button"
                class="inline-flex flex-col items-center justify-center px-5 rounded-tr-lg group hover:bg-blue-800">
...
<script>
document.getElementById('dashboard_nav').addEventListener('click', function() {
...
document.getElementById('files_nav').addEventListener('click', function() {
    document.querySelectorAll('nav a').forEach(function(el) {
        el.classList.remove('pl-10');
    });
    this.classList.add('pl-10');
...
document.getElementById('posts_nav').addEventListener('click', function() {
    document.querySelectorAll('nav a').forEach(function(el) {
        el.classList.remove('pl-10');
    });
    this.classList.add('pl-10');
...
document.getElementById('places_nav').addEventListener('click', function() {
    document.querySelectorAll('nav a').forEach(function(el) {
        el.classList.remove('pl-10');
    });
...
document.getElementById('account_nav').addEventListener('click', function() {
    document.querySelectorAll('nav a').forEach(function(el) {
        el.classList.remove('pl-10');
    });
    this.classList.add('pl-10', 'bg-blue-800');
...
document.getElementById('dashboard_div').addEventListener('click', function() {
...
document.getElementById('files_div').addEventListener('click', function() {
    document.querySelectorAll('div a').forEach(function(el) {
        el.classList.remove('bg-blue-800');
    });
    this.classList.add('bg-blue-800');
...
document.getElementById('posts_div').addEventListener('click', function() {
    document.querySelectorAll('div a').forEach(function(el) {
        el.classList.remove('bg-blue-800');
    });
    this.classList.add('bg-blue-800');
...
document.getElementById('places_div').addEventListener('click', function() {
    document.querySelectorAll('div a').forEach(function(el) {
        el.classList.remove('bg-blue-800');
    });
    this.classList.add('bg-blue-800');
...
document.getElementById('account_div').addEventListener('click', function() {
    document.querySelectorAll('div a').forEach(function(el) {
        el.classList.remove('pl-10');
    });
    this.classList.add('bg-blue-800');
